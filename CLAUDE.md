...
- **Readiness (equipment quality)** — the quality-vs-quantity layer (`game/forces.js`, pure). A force can be **recruited** fully-equipped (readiness 1.0) or **mobilised** with whatever equipment is on hand: mobilise charges full manpower but only an `r`-fraction of each equipment, where `r` = the tightest equipment ratio, blocked below the nation's `equipFloor`. `game.readiness[forceId]` holds the count-weighted pool average, updated by `poolReadiness` on each recruit/mobilise (launch/return don't shift it — same-readiness units leave and come back). At launch, `committedReadiness(need, readiness)` sets the operation's readiness, which (a) multiplies duration via `attritionMult` (slower when under-equipped) and (b) rolls `willFail = Math.random() < failureChance(...)` **once**, baked onto the mission so resolution stays pure. `resolveMissions` reads `m.willFail`: victory → stage+score+forces returned; **defeat** → no stage/score, only `(1 − FAIL_LOSS)` of committed forces survive. Two per-nation traits differentiate play: `equipFloor` (how far under-equipped they'll deploy) and `grit` (higher = gentler attrition + lower defeat threshold — quantity over quality). Set per nation in `nations.js`; defaults 0.5/0.5. The balance simulator recruits fully-equipped, so it never triggers attrition/defeat.
