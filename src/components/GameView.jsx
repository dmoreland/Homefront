...
// The active-campaign UI, composed from the nation config and engine state.
// Presentational: everything it needs arrives via props (App holds the engine).
export function GameView({ nation, game, sim, now, actions }) {
  const shift = nation.upgrades.find((u) => u.tapMult && game.upgrades[u.id]);
  const perTap = (nation.tapBase || 2) * (shift ? shift.tapMult : 1);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#16222E", color: "#EDE6D3", fontFamily: "'Trebuchet MS','Segoe UI',sans-serif", paddingBottom: 32 }}>
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "0 14px" }}>
        ...
        <ForcesList forces={nation.forces} res={game.res} eq={game.eq} owned={game.forces} onRecruit={actions.recruit} />
        <Theatres nation={nation} stages={game.stages} missions={game.missions} forces={game.forces} upgrades={game.upgrades} now={now} onLaunch={actions.launch} />
