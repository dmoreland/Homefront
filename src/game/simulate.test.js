...
describe("simulate — generation", () => {
  ...
  it("produces steel from mills scaled by civilian factories", () => {
    const r = simulate(mk({ owned: { mill: 2, civ: 1 } }), 1);
  ...
  it("multiplies manpower by conscription laws", () => {
    expect(simulate(mk({ upgrades: { law1: true } }), 1).gen.manpower).toBeCloseTo(1.0);
    expect(simulate(mk({ upgrades: { law1: true, law2: true } }), 1).gen.manpower).toBeCloseTo(2.0);
    expect(simulate(mk({ upgrades: { law1: true, law2: true, law3: true } }), 1).gen.manpower).toBeCloseTo(4.0);
  });

  it("boosts steel & aluminium output per Battle of Britain victory", () => {
    const r = simulate(mk({ owned: { mill: 1, smelter: 1 }, stages: { bob: 2 } }), 1);
  ...
  it("adds theatre convoy bonuses to oil & rubber generation", () => {
    const r = simulate(mk({ stages: { africa: 1, atlantic: 2 } }), 1);
...
describe("simulate — production lines", () => {
  it("consumes inputs and produces equipment when fully fed", () => {
    const r = simulate(mk({ owned: { rifleLine: 1 }, res: { steel: 10 } }), 1);
  ...
  it("throttles proportionally and reports STALLED fraction when starved", () => {
    const r = simulate(mk({ owned: { rifleLine: 1 }, res: { steel: 0.5 } }), 1);
  ...
  it("reports net flow as generation minus line consumption", () => {
    const r = simulate(mk({ owned: { mill: 1, rifleLine: 1 }, res: { steel: 10 } }), 1);
...
describe("simulate — oil upkeep", () => {
  it("subtracts air/fleet upkeep and clamps oil at zero", () => {
    const r = simulate(mk({ forces: { fleet: 1 } }), 1);
